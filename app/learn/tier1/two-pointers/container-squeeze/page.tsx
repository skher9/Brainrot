...
import { SQUEEZE_LEVELS } from "@/components/games/tier1/two-pointers/dock/container-squeeze/levels";

const SLUG = "container-squeeze";
...
export default function ContainerSqueezeSelect() {
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ flex: 1, textAlign: "center", fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.16em", color: DOCK.textFaint }}>
          MANIFEST #003 · CONTAINER SQUEEZE
        </div>
          ...
          {SQUEEZE_LEVELS.map((lvl, i) => {
            ...
            return (
              <motion.button
                ...
                onClick={() => unlocked && router.push(`/learn/tier1/two-pointers/container-squeeze/${n}`)}
                style={{
        ...
        {done >= 8 && (
          ...
          >
            <div style={{ fontFamily: "var(--font-display)", fontSize: 20, fontWeight: 900, color: DOCK.cyan, marginBottom: 6 }}>
              MANIFEST 003 SHIPPED ✓
            </div>
            <p style={{ fontFamily: "var(--font-tac)", fontSize: 13, color: DOCK.textDim, lineHeight: 1.7 }}>
              Move the shorter wall. Greedy proof: taller wall move can never improve. O(n).
            </p>
          </motion.div>
        )}
      </div>
