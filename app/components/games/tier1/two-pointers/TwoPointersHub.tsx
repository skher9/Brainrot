...
import { TP_PROBLEMS } from "./problems";
...
export default function TwoPointersHub() {
  ...
  const solved = TP_PROBLEMS.filter(p => {
  ...
  return (
    <div style={{ ...S, display: "flex", flexDirection: "column", gap: 28, padding: "32px 0" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        <div style={{ fontSize: 10, color: "#475569", letterSpacing: "0.12em" }}>TIER 1 · PATTERN 4</div>
        <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700, color: "#e2e8f0", letterSpacing: "-0.01em" }}>
          Two Pointers
        </h1>
        <p style={{ margin: 0, fontSize: 13, color: "#64748b", lineHeight: 1.6 }}>
          8 problems. Two ends meet in the middle. O(n) where O(n²) would suffice.
        </p>
        {!loading && (
      ...
      {!loading && (
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          <div style={{ height: 2, background: "#1a1a1a", borderRadius: 2, overflow: "hidden" }}>
            <div style={{
              height: "100%", background: "#f43f5e", borderRadius: 2,
              width: `${(solved / 8) * 100}%`,
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {TP_PROBLEMS.filter(p => p.index <= 4).map(prob => {
        ...
        {!isPaid && (
          <div style={{
            ...
            <div style={{ fontSize: 12, color: "#64748b", lineHeight: 1.6 }}>
              P5–P8 cover 3Sum, trapping rain water, merge sorted arrays, and 3Sum Closest — the hard interview tier.
            </div>
            <button
...
function ProblemCard({
  ...
  const card = (
    <div style={{
      ...
      padding: "12px 16px",
      background: isSolved ? "rgba(244,63,94,0.04)" : locked ? "rgba(255,255,255,0.01)" : "#111",
      border: `1px solid ${isSolved ? "rgba(244,63,94,0.2)" : locked ? "#161616" : "#1e1e1e"}`,
      borderRadius: 6,
      ...
      <div style={{
        ...
        background: isSolved ? "rgba(244,63,94,0.12)" : "rgba(255,255,255,0.04)",
        border: `1px solid ${isSolved ? "rgba(244,63,94,0.3)" : "#2a2a2a"}`,
        borderRadius: 4,
        display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: 11, fontWeight: 700,
        color: isSolved ? "#f43f5e" : "#475569",
      }}>
  ...
  return (
    <Link href={`/learn/tier1/two-pointers/${prob.index}`} style={{ textDecoration: "none" }}>
      {card}
    </Link>
  );
}
