...
const ContainerSqueeze = dynamic(
  () => import("@/components/games/tier1/two-pointers/dock/container-squeeze/ContainerSqueeze"),
  { ssr: false, loading: () => (
...
export default function ContainerSqueezeLevel({ params }: { params: Promise<{ level: string }> }) {
  const { level } = use(params);
  const router = useRouter();
  const n = parseInt(level, 10);

  if (isNaN(n) || n < 1 || n > 8) {
    router.push("/learn/tier1/two-pointers/container-squeeze");
    return null;
  }
