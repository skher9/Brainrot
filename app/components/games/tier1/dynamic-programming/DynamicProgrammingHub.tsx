...
import { DP_PROBLEMS } from "./problems";
...
export default function DynamicProgrammingHub() {
  ...
  const solved = DP_PROBLEMS.filter(p => {
  ...
  return (
    <div style={{ ...S, display: "flex", flexDirection: "column", gap: 28, padding: "32px 0" }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        <div style={{ fontSize: 10, color: "#475569", letterSpacing: "0.12em" }}>TIER 1 · PATTERN 6</div>
        <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700, color: "#e2e8f0", letterSpacing: "-0.01em" }}>
          Dynamic Programming
        </h1>
        <p style={{ margin: 0, fontSize: 13, color: "#64748b", lineHeight: 1.6 }}>
          8 problems. Fill the table, see the dependencies, watch optimal substructure emerge.
        </p>
        {!loading && (
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {DP_PROBLEMS.filter(p => p.index <= 4).map(prob => {
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        {!isPaid && (
          <div style={{
            ...
            <div style={{ fontSize: 11, color: "#6366f1", letterSpacing: "0.08em" }}>UNLOCK ADVANCED PROBLEMS</div>
            <div style={{ fontSize: 12, color: "#64748b", lineHeight: 1.6 }}>
              P5–P8 cover 0/1 knapsack, unique paths, edit distance, and longest palindromic subsequence.
            </div>
            <button
...
function ProblemCard({
  ...
  const card = (
    <div style={{
      ...
      padding: "12px 16px",
      background: isSolved ? `rgba(129,140,248,0.04)` : locked ? "rgba(255,255,255,0.01)" : "#111",
      border: `1px solid ${isSolved ? "rgba(129,140,248,0.2)" : locked ? "#161616" : "#1e1e1e"}`,
      borderRadius: 6,
      ...
      <div style={{
        ...
        background: isSolved ? "rgba(129,140,248,0.12)" : "rgba(255,255,255,0.04)",
        border: `1px solid ${isSolved ? "rgba(129,140,248,0.3)" : "#2a2a2a"}`,
        borderRadius: 4,
        display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: 11, fontWeight: 700,
        color: isSolved ? ACCENT : "#475569",
      }}>
  ...
  return (
    <Link href={`/learn/tier1/dynamic-programming/${prob.index}`} style={{ textDecoration: "none" }}>
      {card}
    </Link>
  );
}
