...
const SushiBelt = dynamic(
  () => import("@/components/games/tier1/sliding-window/scan/sushi-belt/SushiBelt"),
  { ssr: false, loading: () => (
...
export default function SushiBeltLevel({ params }: { params: Promise<{ level: string }> }) {
  const { level } = use(params);
  const router = useRouter();
  const n = parseInt(level, 10);

  if (isNaN(n) || n < 1 || n > 8) {
    router.push("/learn/tier1/sliding-window/sushi-belt");
    return null;
  }
  ...
  return (
    <SushiBelt
      ...
      onBack={() => router.push("/learn/tier1/sliding-window/sushi-belt")}
    />
  );
}
