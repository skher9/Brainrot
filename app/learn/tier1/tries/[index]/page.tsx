  ...
  return (
    <div style={{ ...S, height: "100vh", background: "#0a0a0a", display: "flex", flexDirection: "column", overflow: "hidden", position: "relative" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 20px", borderBottom: "1px solid #111", flexShrink: 0, zIndex: 10 }}>
        <Link href="/learn/tier1/tries" style={{ textDecoration: "none" }}>
          <span style={{ fontSize: 10, color: "#374151", cursor: "pointer", letterSpacing: "0.08em" }}>← TRIE</span>
        </Link>
        <button onClick={() => { setAlgoAutoClose(false); setAlgoModalOpen(true); }} style={{ padding: "3px 10px", background: "rgba(6,182,212,0.08)", border: "1px solid rgba(6,182,212,0.25)", borderRadius: 4, cursor: "pointer", fontSize: 9, color: ACCENT, fontFamily: "inherit", letterSpacing: "0.1em" }}>
          LEARN ALGO
        </button>
        ...
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "#080c10", border: "1px solid #0d2040", borderBottom: "none", borderTopLeftRadius: 10, borderTopRightRadius: 10, padding: "22px 24px 28px", zIndex: 50, fontFamily: "inherit", animation: "slideUp 0.35s cubic-bezier(0.16, 1, 0.3, 1) both" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 580, margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              ...
              {idx < 8 && (
                <Link href={`/learn/tier1/tries/${idx + 1}`} style={{ textDecoration: "none" }}>
                  <button style={{ padding: "8px 16px", background: "rgba(6,182,212,0.1)", border: "1px solid rgba(6,182,212,0.4)", borderRadius: 4, cursor: "pointer", fontSize: 10, color: ACCENT, fontFamily: "inherit", letterSpacing: "0.06em" }}>NEXT →</button>
                </Link>
              )}
            </div>
          </div>
