...
const CargoMatch = dynamic(
  () => import("@/components/games/tier1/two-pointers/dock/cargo-match/CargoMatch"),
  { ssr: false, loading: () => (
...
export default function CargoMatchLevel({ params }: { params: Promise<{ level: string }> }) {
  const { level } = use(params);
  const router = useRouter();
  const n = parseInt(level, 10);

  if (isNaN(n) || n < 1 || n > 8) {
    router.push("/learn/tier1/two-pointers/cargo-match");
    return null;
  }
