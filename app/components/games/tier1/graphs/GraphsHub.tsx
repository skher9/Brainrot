...
import { GRAPH_PROBLEMS } from "./problems";
...
export default function GraphsHub() {
  ...
  const solved = GRAPH_PROBLEMS.filter(p => {
  ...
  return (
    <div style={{ ...S, display: "flex", flexDirection: "column", gap: 28, padding: "32px 0" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        <div style={{ fontSize: 10, color: "#475569", letterSpacing: "0.12em" }}>TIER 1 · PATTERN 3</div>
        <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700, color: "#e2e8f0", letterSpacing: "-0.01em" }}>
          Graphs &amp; Flood Fill
        </h1>
        <p style={{ margin: 0, fontSize: 13, color: "#64748b", lineHeight: 1.6 }}>
          8 games. Explore, spread, find paths.
        </p>
        {!loading && (
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {GRAPH_PROBLEMS.filter(p => p.index <= 4).map(prob => {
        ...
        {!isPaid && (
          <div style={{
            ...
            <div style={{ fontSize: 12, color: "#64748b", lineHeight: 1.6 }}>
              P5–P8 cover multi-source BFS, reverse flow DFS, BFS on implicit graphs, and Tarjan's bridge algorithm — the hard interview tier.
            </div>
            <button
...
function ProblemCard({
  ...
  return (
    <Link href={`/learn/tier1/graphs/${prob.index}`} style={{ textDecoration: "none" }}>
      {card}
    </Link>
  );
}
