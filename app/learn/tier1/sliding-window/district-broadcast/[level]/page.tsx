...
const DistrictBroadcast = dynamic(
  () => import("@/components/games/tier1/sliding-window/scan/district-broadcast/DistrictBroadcast"),
  { ssr: false, loading: () => (
...
export default function DistrictBroadcastLevel({ params }: { params: Promise<{ level: string }> }) {
  const { level } = use(params);
  const router = useRouter();
  const n = parseInt(level, 10);

  if (isNaN(n) || n < 1 || n > 8) {
    router.push("/learn/tier1/sliding-window/district-broadcast");
    return null;
  }
  ...
  return (
    <DistrictBroadcast
      ...
      onBack={() => router.push("/learn/tier1/sliding-window/district-broadcast")}
    />
  );
}
