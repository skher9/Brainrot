...
import { CARGO_LEVELS } from "@/components/games/tier1/two-pointers/dock/cargo-match/levels";

const SLUG = "cargo-match";
...
export default function CargoMatchSelect() {
          ...
          {CARGO_LEVELS.map((lvl, i) => {
            ...
            return (
              <motion.button
                ...
                onClick={() => unlocked && router.push(`/learn/tier1/two-pointers/cargo-match/${n}`)}
                style={{
                  textAlign: "left", padding: "16px", borderRadius: 10,
                  background: completed
                    ? "rgba(0,100,80,0.08)"
                    : isNext ? "rgba(0,80,160,0.09)" : "rgba(8,14,26,0.6)",
                ...
                {unlocked && (
                  <>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      {isNext && (
                        <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: DOCK.blue, letterSpacing: "0.12em" }}>PLAY →</span>
                      )}
                      {completed && (
                        <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: DOCK.green }}>CLEARED</span>
                      )}
                    </div>
                  </>
                )}
