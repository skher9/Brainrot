...
import { SW_PROBLEMS } from "./problems";
...
export default function SlidingWindowHub() {
  ...
  const solved = SW_PROBLEMS.filter(p => {
  ...
  return (
    <div style={{ ...S, display: "flex", flexDirection: "column", gap: 28, padding: "32px 0" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        <div style={{ fontSize: 10, color: "#475569", letterSpacing: "0.12em" }}>TIER 1 · PATTERN 5</div>
        <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700, color: "#e2e8f0", letterSpacing: "-0.01em" }}>
          Sliding Window
        </h1>
        <p style={{ margin: 0, fontSize: 13, color: "#64748b", lineHeight: 1.6 }}>
          8 problems. A frame that slides, expands, and contracts. O(n) over nested loops.
        </p>
        {!loading && (
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {SW_PROBLEMS.filter(p => p.index <= 4).map(prob => {
        ...
        {!isPaid && (
          <div style={{
            ...
            <div style={{ fontSize: 12, color: "#64748b", lineHeight: 1.6 }}>
              P5–P8 cover sliding window maximum, minimum window substring, character replacement, and find all anagrams.
            </div>
            <button
...
function ProblemCard({
  ...
  const card = (
    <div style={{
      ...
      padding: "12px 16px",
      background: isSolved ? `rgba(16,185,129,0.04)` : locked ? "rgba(255,255,255,0.01)" : "#111",
      border: `1px solid ${isSolved ? "rgba(16,185,129,0.2)" : locked ? "#161616" : "#1e1e1e"}`,
      borderRadius: 6,
      ...
      <div style={{
        ...
        background: isSolved ? "rgba(16,185,129,0.12)" : "rgba(255,255,255,0.04)",
        border: `1px solid ${isSolved ? "rgba(16,185,129,0.3)" : "#2a2a2a"}`,
        borderRadius: 4,
  ...
  return (
    <Link href={`/learn/tier1/sliding-window/${prob.index}`} style={{ textDecoration: "none" }}>
      {card}
    </Link>
  );
}
