...
import { SIGNAL_LEVELS } from "@/components/games/tier1/sliding-window/scan/signal-scanner/levels";
...
export default function SignalScannerSelect() {
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 600, margin: "0 auto", padding: "32px 20px 140px" }}>
        ...
        >
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.3em", color: SCAN.green, marginBottom: 8 }}>◆ SIGNAL SCANNER</div>
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: 28, fontWeight: 900, color: SCAN.text, marginBottom: 16 }}>SELECT SIGNAL</h1>

          <div style={{ padding: "12px 16px", background: "rgba(10,20,16,0.7)", border: `1px solid ${SCAN.border}`, borderRadius: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, letterSpacing: "0.16em", color: SCAN.textFaint }}>SIGNALS CLEARED</span>
          ...
          {SIGNAL_LEVELS.map((lvl, i) => {
            ...
            return (
              <motion.button
                ...
                onClick={() => unlocked && router.push(`/learn/tier1/sliding-window/signal-scanner/${n}`)}
                style={{
                ...
                {unlocked && (
                  <>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      {isNext && <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: SCAN.green, letterSpacing: "0.12em" }}>SCAN →</span>}
                      {completed && <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: SCAN.green }}>CLEARED</span>}
                    </div>
                  </>
                )}
              </motion.button>
        ...
        {done >= 8 && (
          ...
          >
            <div style={{ fontFamily: "var(--font-display)", fontSize: 20, fontWeight: 900, color: SCAN.green, marginBottom: 6 }}>
              CASE FILE 001 CLOSED ✓
            </div>
            <p style={{ fontFamily: "var(--font-tac)", fontSize: 13, color: SCAN.textDim, lineHeight: 1.7 }}>
              Fixed window: subtract left, add right. O(n) from O(n·k). One pass wins.
            </p>
          </motion.div>
        )}
      </div>
