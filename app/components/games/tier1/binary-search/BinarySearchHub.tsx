...
import { BS_PROBLEMS } from "./problems";
...
export default function BinarySearchHub() {
  ...
  const solved = BS_PROBLEMS.filter(p => {
    ...
    <div style={{ ...S, display: "flex", flexDirection: "column", gap: 28, padding: "32px 0" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        <p style={{ margin: 0, fontSize: 13, color: "#64748b", lineHeight: 1.6 }}>
          8 games. Halve the search space every step.
        </p>
        {!loading && (
        ...
        {!isPaid && (
          <div style={{
            ...
            <div style={{ fontSize: 12, color: "#64748b", lineHeight: 1.6 }}>
              P5–P8 cover rotated arrays, minimum-speed search, capacity optimization, and median of two sorted arrays — the hard interview tier.
            </div>
            <button
...
function ProblemCard({
  ...
  return (
    <Link href={`/learn/tier1/binary-search/${prob.index}`} style={{ textDecoration: "none" }}>
      {card}
    </Link>
  );
}
